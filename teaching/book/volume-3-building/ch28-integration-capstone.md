...
> **上一章：[第 27 章 高级扩展](./ch27-advanced-extension.md)**
...
> **参考答案**：
>
> 1. **只需改初始化代码**，把 `SQLiteMemory(...)` 换成 `InMemoryMemory()`。测试用例本身不需要修改——因为两者都实现 `MemoryBase` 接口，`add`、`get_memory`、`size` 等方法签名完全一致。这就是抽象层的价值：依赖接口而非实现，底层可以自由替换。唯一的差异是序列化行为：`InMemoryMemory` 的 `state_dict()` 会序列化全部消息数据，而 `SQLiteMemory` 只保存数据库路径。
> 2. **控制变量**。如果测试包含了真实的模型调用，失败原因可能来自模型（幻觉、格式错误、拒绝回答）也可能来自工具链路——难以定位问题。跳过模型层直接构造 `ToolUseBlock`，可以**隔离测试工具注册 → 参数解析 → 函数执行 → 返回格式化**这条链路，确保工具系统本身正确工作。模型行为的变化不会影响这些测试的稳定性。
